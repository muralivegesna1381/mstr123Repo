...
import FillGradiantImg from "./../../../assets/images/otherImages/svg/filterGradientImg.svg";
import FilterIcon from "./../../../assets/images/otherImages/svg/filterIcon.svg";
import FilterBckView from "./../../../assets/images/otherImages/png/bgTimerFilter.png";
...
const FeedbackUI = ({ route, ...props }) => {
    ...
    const filterData = async () => {
        ...
        if (toDate && fromDate) {
            ...
            data = feedbackArray.filter((item) => lowerLimit <= new Date(moment.utc(item.feedbackDate).toDate()).getTime() && new Date(moment.utc(item.feedbackDate).toDate()).getTime() <= upperLimit)
        ...
        return (
            <TouchableOpacity key={index} style={{ padding: 1 }} onPress={() => { actionOnRow(item) }}>
                <View style={styles.cellBackViewStyle}>
                    ...
                    <Text numberOfLines={2} style={[styles.tdTextextStyle, { flex: 2 }]}>{item && item.feedbackDate ? "" + moment(moment.utc(item.feedbackDate).toDate()).local().format('MM-DD-YYYY') : ''}</Text>
                </View>
            </TouchableOpacity>
        );
    };
    ...
    return (
        <View style={[styles.mainComponentStyle]}>
            ...
            <View style={styles.listViewStyle}>
                ...
                {(!noLogsShow && !props.isLoading) || feedbackArray.length > 0 ? <View style={styles.mainViewStyle}>
                    ...
                    <TouchableOpacity style={styles.filterBtnStyle} onPress={() => { set_ListOpen(!isListOpen) }}>
                        ...
                        <View onLayout={(event) => {
                            ...
                            <FilterIcon style={[styles.filterIconStyle]} width = {wp('4%')} height = {hp('4%')} imageStyle={{ borderRadius: 5 }} />
                ...
                {!noLogsShow && !props.isLoading ? <View style={styles.headingView}>
                ...
                {!noLogsShow ? <View style={styles.recordListStyle}>
                    ...
                    (!props.isLoading ? <View style={{ justifyContent: 'center', alignItems: 'center', marginTop: hp("15%"), }}>
                        <DogCatImg style={[styles.nologsDogStyle, { width: wp("55%"), height: hp("25%"), resizeMode: 'contain' }]}/>
                ...
                {isListOpen ? <View style={[styles.timerFilterListStyle, { top: dropDownPostion.y + dropDownPostion.height },]}>
                    ...
                    <ImageBackground style={{ alignItems: "center", justifyContent: "center", }} imageStyle={{ borderRadius: 25 }} source={FilterBckView}>
